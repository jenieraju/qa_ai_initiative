...
- **Case-type vocabulary is closed, and matches `ui-test-design` exactly.** Use exactly one of: `happy-flow`, `negative-validation`, `boundary`, `auth-state`, `visual-layout`, `accessibility`, `responsive-cross-browser`. Don't invent new categories, and don't flag a case type as "missing" for a page/flow it genuinely doesn't apply to (e.g. `auth-state` on a page whose `Auth required` column is `None`, or `visual-layout` when the project has no visual-regression tool wired in at all).
...
- **No credentials in the output.** If a gap involves an auth-related case, describe the state (e.g. "no `auth-state` row found for this session-gated page") — never write an actual token, cookie value, or password.
...
Every page/flow expects `happy-flow` (there's always a primary success path to check). Beyond that, applicability is conditional and must cite the column/source that justifies it:

| Case type | Expected when | Justified by |
|---|---|---|
| `happy-flow` | Always | n/a |
| `negative-validation` | Page/flow has any form input, field, or user-editable control | `ui-context.md`'s Key components column shows input fields/forms |
...
| `auth-state` | Auth required is anything other than `None` | `ui-context.md`'s Auth required column. If `Unknown`, don't guess either way — note under Open Questions that applicability can't be judged until `get-ui-context` resolves it |
| `visual-layout` | Project already has a visual-regression tool and baseline images wired in | Existing test/framework setup section of `ui-context.md`, or the matrix's own rows. If no such tooling exists, `visual-layout` is not applicable for any page — never flag it missing project-wide, per the guardrail above |
| `accessibility` | Project already has an accessibility-testing tool wired in | Same existing test/framework setup source. If none exists, not applicable for any page |
| `responsive-cross-browser` | Multiple viewports/browsers are documented as in-scope for this project | The UI agent's Project config (browsers/viewports field) or `ui-context.md`'s existing test/framework setup section. If only one browser/viewport is in scope, not applicable |
...
8. **Flag everything else under Open Questions** (never as fabricated rows): unresolved `Auth required: Unknown` pages, unresolved boundary-constraint sources, low-confidence rule matches, orphaned matrix rows, case types skipped project-wide for lack of tooling (visual-layout/accessibility) so a human can confirm that's intentional rather than an oversight.
...
Models tend to stop at "does this page appear in the matrix at all" and call that coverage — missing the case-type-level and business-rule-level gaps that actually matter (a page can have five happy-flow rows and zero negative-validation cases and still read as "covered"). The opposite bias is over-flagging: mechanically listing all seven case types as expected for every page regardless of whether the project actually supports the applicability (e.g. flagging `visual-layout` missing when no visual-regression tool exists anywhere in the project, or `auth-state` missing on a `None`-auth public page). Force every "expected" judgment through the applicability table above, both to find real gaps and to avoid noise from ones that don't apply.
...
|---|---|---|---|---|---|---|---|---|
| 1 | LoginPage | `/login` | None | happy-flow, negative-validation | happy-flow, negative-validation, boundary | `boundary` | Partial | `<e.g. "boundary unresolved — no documented constraint found">` |
...
- The seven case types and the applicability table are the fixed benchmark across every project; what varies is which pages/flows/rules actually exist, and which case types even apply (visual-layout/accessibility/responsive-cross-browser depend entirely on what tooling the project has wired in).
...
- If `ui-test-design`'s final case-type vocabulary ever changes from the seven listed here, update this skill's closed vocabulary and applicability table to match — never silently drift out of sync with the matrix's own `Case type` column.
